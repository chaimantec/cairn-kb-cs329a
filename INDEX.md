...
> **Coverage note — partial.** This knowledge base covers **catalog lectures 1–5 of 9**
> (Course Overview; Test-Time Compute Scaling; Robust Verification; Learning from Feedback with
> Tools/Code; Planning and Multi-Step Reasoning). It says nothing about any later lecture. Do not cite it as covering the course.
>
...
> non-exclusive) are linked only, and the recording discusses only LATS, SPRINT and SWiRL. Figures are the papers' own, shown with their printed captions; this KB
...
> row 7, row 8 or both. Positions 2, 3, 4 and 5 are confirmed against their transcripts; everything after
...
- [Test-time scaling](wiki/test-time-scaling.md) — repeated sampling with a verifier, coverage vs
  ...
  (LATS), and running the independent parts of one reasoning trace in parallel (SPRINT).
- [Verifiers](wiki/verifiers.md) — what a verifier does, unit tests and other verifiable domains,
  ...
  as a search value (LATS) and a step-wise process reward (SWiRL).
...
- [Reasoning models](wiki/reasoning-models.md) — the thinking behaviours (analysis, decomposition,
  ...
  on why they work, and SPRINT's training of a reasoning model to plan and execute in parallel.
- [The LLM training pipeline](wiki/llm-training-pipeline.md) — pre-training, fine-tuning on
  ...
  replaces human harmlessness labels with AI feedback, and SWiRL's step-wise RL for multi-step tasks.
...
- [Self-improvement](wiki/self-improvement.md) — the course's central idea: test-time generation as
  ...
  trajectories (SWiRL), and what is not yet understood about RL.
