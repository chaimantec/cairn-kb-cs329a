...
- [x] 07 — raw/papers/07-alphacode.md (main body only; Sonnet; 9 sections, 26 subsections, 6 subsubsections, 13 figures — Figure 5 as a code block, no image — 13 tables; check_math clean, 107 formulas). check_paper_file.py --main-only --no-crop 5 is misled by this paper's `\appendixref` macro: its first `\appendix` match is in the preamble, which empties the body. Re-run on a scratch copy with the macro renamed: sections, subsections, figure/table environments, images, macros, leftovers all ok. 3 FAILs, all confirmed artifacts — 10 "missing" table numbers are `\includegraphics` widths inside tabular-wrapped figures (0.48 ×4, 0.60, 0.35, 0.42, 0.45, 0.3) and a `1549` image path; 20 "unfound" LaTeX paragraphs are the omitted back matter (Acknowledgements, 11 Author Contributions paragraphs, Data availability), Figure 2's in-figure problem statement (shown by its image) and 5 table rows/headers carrying `\cellcolor` or `\multirow` markup; 7 unmatched markdown paragraphs are the spec's italic header-flattening notes. Quirks kept: "10@10K" vs "10@1k" as printed; appendix cross-references given their printed "Appendix Figure/Table A#" labels
- [x] 07 — AlphaCode 2 and Search-o1: linked, discussed and cited only (licence); no transcription, no images
- [x] 08 — mapping confirmed: the transcript names all three row-17 readings at ≈1:38 and discusses them in order (METR ≈3:56–27:25, GDPval ≈27:25–51:40, DeepScholar-Bench ≈51:40–1:01:03)
- [x] 08 — licences checked on the arXiv abstract pages (2026-09-15): all three CC BY 4.0; no rights-reserved notice in the PDFs
- [x] 08 — versions: METR is now v4 (Jul 2026, retitled "…Long Software Tasks") and DeepScholar-Bench v2 (Feb 2026), but the lecture (Nov 17, 2025) quotes METR v2 and DeepScholar-Bench v1 (Claude 3.7 Sonnet 59 min / 15 min, DeepScholar-base). User chose the lecture's versions: e-prints and PDFs for 2503.14499v2 and 2508.20033v1 fetched to raw/pdfs/papers/ and src/ (gitignored). GDPval has only v1
- [x] 08 — figure crops from the transcribed versions' main bodies → raw/images/08-agentic-evaluations-and-long-horizon-tasks/: metr-figure-1–13 (v2), gdpval-figure-1–9, deepscholar-figure-1–3 (v1) (25, 3.7MB). METR Figure 9 (no graphics found) and GDPval Figure 2 (box cut the paragraph above) re-cropped with --clip; text-layer check clean on all 25
- [x] 08 — raw/papers/08-metr-long-tasks.md (METR v2 main body; Sonnet; 498 lines, 8 sections, 17 subsections, 13 figures with images, 3 tables, 20 footnotes; appendix cross-references resolved to printed labels from the PDF text layer). check_paper_file.py --main-only: sections, figures, tables, images, macros, leftovers all ok. The first run also failed wording on 59 paragraphs: the checker's `\[...\]` display-math stripper matched the `\\[2ex]` line break in the author block and deleted everything up to §4.1's equation. Parent confirmed it on a scratch copy (59 → 1) and patched the skill's check_paper_file.py to ignore an escaped `\\[`; the re-run on the real source gives 1. Two FAILs remain, both confirmed artifacts: 2 "missing" table numbers are Table 1's column widths (0.15, 0.55), and the 1 unmatched paragraph is Table 1's caption, whose `\citet`s the checker strips on the LaTeX side but only de-years on the markdown side. check_math 13 formulas, 0 errors, 0 warnings. Footnote 14's regression formula is plain text ("log(model horizon) = α + β · release date"), because GitHub does not render math in footnote definitions. Paper typos kept verbatim: "Bootstap", "See See Appendix", "messiness""", and 169 vs 170 tasks in Table 2's caption
- [x] 08 — raw/papers/08-gdpval.md (main body; Sonnet; 245 lines, 6 sections, 9 subsections, 9 figures with images — Figures 4 and 9 with their subcaptions, Figures 2 and 3 numbered by PDF order since they have no `\label` — 1 table, 5 footnotes; all 12 appendix cross-references resolved to printed labels from the PDF text layer, consistent with the wiki's appendix citations). check_paper_file.py --main-only: every check passes except 1 confirmed artifact, 2 "missing" table numbers that are Table 1's column widths (1.5cm, 10cm). Wording: 47/47 LaTeX paragraphs found, 41/41 markdown paragraphs traced. check_math 0 errors, 0 warnings; every literal dollar escaped. The source's citation key brynjolfsson2019artificial prints as 2017 in the .bbl and PDF, and 2017 is used
- [x] 08 — raw/papers/08-deepscholar-bench.md (DeepScholar-Bench v1 main body; written inline by the parent after two subagent attempts were killed by API errors — rate limit, then org access disabled for subagents. 357 lines, 7 sections, 9 subsections, 6 subsubsections, 3 figures with images (Figure 1 with its subcaptions), 6 tables, 1 footnote; appendix cross-references resolved to the printed labels 'Appendix Section 8.2/8.3/8.4' and 'Box 7/8/9' from the PDF text layer — the appendix sections print as 8.x because acmart continues the section counter). check_paper_file.py --main-only (on a scratch copy with `\appendix` inserted, since v1's main.tex has none): sections, subsections, figures, tables, images, table numbers (285/285), wording (78/78 LaTeX paragraphs) and macros all pass; 2 FAILs are the spec's italic table-flattening notes under Tables 2 and 5, the same known artifact as lectures 4–5. check_math clean (60 formulas). The paper prints numeric citations; this file resolves them to author–year from the .bbl (the header says so), and the paper's no-author citations (product/model/dataset pages — OpenAI, Gemini, Claude, Grok, Perplexity, arXiv, OpenAlex, LOTUS, Llama-4, the OpenAI model pages) are left as the names the prose already gives them, since their numbered entries are omitted; the one cite not named in prose is rendered '(DeepConsult, n.d.)'. Two Rahmani and two Upadhyay cites disambiguated as 2024a/b per the .bbl's natexlab. Source typos kept verbatim: 'disticnt', 'filter's out', 'disclude', 'it's title', 'evalution', 'Overall,the', 'shows to oracle settings', 'form ArXiv and once', 'rspective', 'retreival', 'whcih', 'notions correctness', 'Citation Recall', 'taviliy.com', 'Search AI's'. Wiki/08's Table 2 and Table 3 excerpts match the full tables
...
- [x] INDEX.md — lecture 8, coverage 8 of 9, readings and versions note, positions 2–8 confirmed, topic one-liners, raw/papers and raw/images
- [x] sources.md row 17 (licences, versions, full-text links), schedule table, "ingested so far", mapping paragraph; AGENTS.md (licences, version rule, images, crop note, numbering); kb.json (coverage 8, 14 readings, 101 images, lecture 8 caveats, mapping caveat) — to re-verify once the three paper files exist
...
- [x] raw/images/08-agentic-evaluations-and-long-horizon-tasks/ — 25 main-body figure crops (METR v2 1–13, GDPval 1–9, DeepScholar-Bench v1 1–3); 14 embedded in wiki/08, all 25 to be embedded in the paper files
...
- [x] Lecture 7: kb.json updated (coverage 7, 11 topic pages, 11 readings ingested, 76 images, lecture 7 licence/discrepancy/garble caveats, mapping caveat resolved), sources.md rows 7–8, AGENTS.md, verify_kb.py clean (all hard checks, GitHub math included) and review read — numeric claims (images only in lectures 2–5 and 7; AlphaCode Figures 1–4 and 6–13 = 12 on disk, 6 in wiki, all 12 in the paper file) match the measured distribution; committed and pushed (kbUrl already set)
- [x] Lecture 8: verify_kb.py clean and review read — all hard checks pass (INDEX links, relative links to tracked files, kb.json counts 8/8/8 lectures-transcripts-pages, 101 images byLecture, every formula on github.com), and the review section read: the 4 numeric prose claims (INDEX and AGENTS image-coverage sentences) match the measured distribution (lectures 2-5, 7 and 8 have images; lecture 8: 25 on disk, 14 in the wiki, all 25 in the paper files); check_math 1251 formulas, 0 errors, 0 warnings on both renderers; committed and pushed (kbUrl already set)
- [x] Lecture 9: kb.json updated (coverage 9 of 9, 28 caveats — the three-papers-not-readings caveat, the transcript-only caveat, the caption ambiguities; mapping resolved to row 20 by content), sources.md row 20 + a Row 20 section, AGENTS.md (position 9 confirmed, the papers-that-are-not-readings rule), INDEX.md coverage note rewritten to complete; verify_kb.py clean (all hard checks, 1276 formulas on both renderers) and its review read — the numeric prose claims match the measured image distribution (images in lectures 2–5, 7 and 8; lecture 9 has none); committed and pushed (kbUrl already set, confirmed on the catalog entry). **This completes the KB: 9 of 9 lectures.**
